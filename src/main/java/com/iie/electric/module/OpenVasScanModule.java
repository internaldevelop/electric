...
import com.iie.electric.service.TaskResultService;
...
public class OpenVasScanModule {
    ...
    public void handleOpenVasResult(int projectID, int taskID, String ip, String result) {
        JSONObject jsonIpInfo = new JSONObject(result.substring(7));//取出ipInfo字段
        if (!jsonIpInfo.has("progress")) {
            ...
            JSONObject jsonNewIpInfo = new JSONObject(newIpInfo);
    ...
    private void removeOpenVasNull(JSONArray openVasVulArr) {
        for (int i = 0; i < openVasVulArr.length(); i++) {
