package com.iie.electric.module;
...
import com.iie.electric.entityview.ReportResultCount;
import com.iie.electric.service.*;
import com.iie.electric.util.ReportUtil;
import org.json.JSONException;
import org.json.JSONObject;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.iie.electric.entity.Project;
import com.iie.electric.entity.Task;
import com.iie.electric.util.Constant;
import com.iie.electric.util.LogUtil;
...
//            cmd += " " + "--SD-addition-port" + " " + "T:102 502 9600 2404 20000" + " " + "U:" + project.getSdAdditionPortUdp() + " 161";
//        }
//        if (!project.getSdAdditionPortTcp().equals("") && !project.getSdAdditionPortUdp().equals("")) {
//            cmd += " " + "--SD-addition-port" + " " + "T:102 502 9600 2404 20000" + " " + project.getSdAdditionPortTcp() + " " + "U:" + project.getSdAdditionPortUdp() + " U161";
//        }
//        if (!project.getSdAdditionPortTcp().equals("") && !project.getSdAdditionPortUdp().equals("")) {
//            cmd += " " + "--SD-addition-port" + " " + "T:" + project.getSdAdditionPortTcp() + " " + "U:" + project.getSdAdditionPortUdp();
//        }else{
//            cmd += " " + "--SD-addition-port" + " " + "T:102 502 9600 2404 20000";
//        }
//        if (!project.getSdAdditionPortTcp().equals("") && project.getSdAdditionPortUdp().equals("")) {
    ...
    public boolean handleTaskResult(int taskid, int projectid, String taskResult) {
        try {
            ...
            if (taskResult.indexOf("@error") == 0) {
                ...
                JSONObject resultJson = new JSONObject(errorResult);//把结果转化为json对象
                int num = resultJson.getInt("NO");
                String msg = resultJson.getString("msg");
                strError = num + msg + "#";
                if (resultJson.getInt("NO") >= 300) {
                    injectCmd("stop");
                }
            }
            if (taskResult.indexOf("@output") == 0) {
                String resultStr = taskResult.substring(7);//取出任务执行结果
                JSONObject resultJson = new JSONObject(resultStr);//把结果转化为json对象
                if (resultJson.has("progress")) {//如果是任务进度
                    int progress = resultJson.getInt("progress");
