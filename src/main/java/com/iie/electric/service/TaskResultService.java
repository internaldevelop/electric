...
import com.iie.electric.dao.TaskResultDao;
...
public class TaskResultService extends BaseService {
    ...
    private String addField(String process, JSONObject ipInfoJson) throws JSONException {
        if (process.equals("VS")) { // 漏洞验证结果中添加漏洞信息
            ...
            if (deviceSumObj != null) {
                JSONArray deviceInfoList = deviceSumObj.getJSONArray("device_info_list");
                if (deviceInfoList != null) {
                    JSONObject jsonObject = deviceInfoList.getJSONObject(0);
                    if (jsonObject != null && jsonObject.getString("brand").equals("")) {
                        JSONObject macInfo = ipInfoJson.getJSONObject("mac_info");
                        if (macInfo != null) {
                            jsonObject.put("brand", macInfo.getString("vendor"));
                        }
                    }
                }
            }
            ...
            if (portList != null) {
                for (int i = 0; i < portList.length(); i++) {
                    JSONObject jsonObject = portList.getJSONObject(i);
    ...
    private void addVulInfo(JSONArray vulArray) throws JSONException {
        ...
        for (int i = 0; i < vulArray.length(); i++) {
            ...
            if (vulJsonObj.getInt("checked") == 1) {
        ...
        for (String ip : taskResultMap.keySet()) {
            ...
            if (portInfoList != null) {
                for (PortInfo portInfo : portInfoList) {
                    ...
                    String serviceName = "";
                    com.iie.electric.entity.Service service = portInfo.getService();
                    if (service != null && service.getType() != null) {
                        serviceName = service.getType();
                    }
                    String tmpService = "";
                    if (serviceName.length() == 0) {
                        tmpService = type + ":" + port;
                    } else {
                        tmpService = serviceName + ":" + port;
                    }
                    if (serviceList.length() == 0) {
                        serviceList = tmpService;
                    } else {
                        serviceList += ";" + tmpService;
                    }
                }
            ...
            deviceOverView.setServiceList(serviceList);
    ...
    public ExploitVerifyParam getExploitVertifyParam(int projectID, int taskID, String ip, String vulName) {
        ...
        if (ipInfo != null) {
            ...
            JSONObject ipInfoJson = new JSONObject(ipInfo);
