...
var Table = React.createClass( {displayName: "Table",
    ...
    render: function() {
        ...
        return React.createElement("table", {className: "table table-bordered table-hover userTable"}, 
...
function init() {
    ...
    $( '.table' ).delegate( '.account', 'click', function() {
        ...
        var userType = $( '.ps', $parentTr ).text() == "审计员"?"audit":($( '.ps', $parentTr ).text() == "管理员"?"admin":"checker");
        $("select[name='userType']").val(userType ).attr( 'disabled', 'disabled' );
        $ps.val( '' );
        $confirmPs.val( '' );
 ...
 */
    $( '#addOrChangeModel .submit' ).click( function() {
        ...
        if( type === 'add' ) {
            successMsg = '添加成功!';
            errorMsg =['添加失败','用户名已存在','密码必须是数字、字符、特殊字符(~!@#$%^&*_)组合，且大于8位','新密码与确认密码不一致','密码不能包含用户名','session过期','已删除的用户不能再次添加']
            console.log(param);
            getRsaKey(param , 'add_checker' , successMsg ,errorMsg);
        } else if( type === 'account' ){
            successMsg = '修改成功!';
        errorMsg =['修改失败','检查员不存在','密码必须是数字、字符、特殊字符(~!@#$%^&*_)组合，且大于8位','新密码与确认密码不一致','密码不能包含用户名','session过期','已删除的用户不能再次添加']
...
/******************************修改检查员权限结束******************************************/
    ...
    $.validator.addMethod("userlong",function(value,element,params){
    ...
    $.validator.addMethod("long",function(value,element,params){
